...
using lozzaxd_seckeys = std::tuple<legacy_seckey, ed25519_seckey, x25519_seckey>;

// Synchronously retrieves SN private keys from lozzaxd via the given oxenmq address.  This constructs
...
// false.
lozzaxd_seckeys get_sn_privkeys(std::string_view lozzaxd_rpc_address, std::function<bool()> keep_trying = nullptr);
